...
(function () {
    ...
    const secondaryCarousels = [ ...document.querySelectorAll(".has-slider").values() ];

    if (secondaryCarousels.length > 0) {
        secondaryCarousels.forEach(secondaryCarousel => {
            const slidesContainer = secondaryCarousel.querySelector(".is-slides-container");
            ...
            slides.forEach((el, idx, arr) => {
                ...
                carousel.on("swipe", (e) => {
                    ...
                    case SWIPE_DIRECTION.left:
                        if (idx < arr.length - 1) {
                            slidesContainer.style.transform = `translateX(-${(idx + 1) * 100}vw)`;
                            [ ...slidesControls.querySelectorAll(".ee-section__slide-control-button").values() ].forEach((el) => {
                                el.classList.remove("active");
                            });
                            [ ...slidesControls.querySelectorAll(".ee-section__slide-control-button").values() ][ idx + 1 ].classList.add("active");
                        }
                        break;
